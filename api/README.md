...
| POST | `/v1/photo?c=slug` | по токену | загрузить снимок блюда |
| GET | `/v1/photo/slug/имя` | любой | отдать снимок, кэш навсегда |
| GET | `/v1/health` | любой | проверить, что воркер жив |

## Снимки блюд

Заведение фотографирует блюдо на телефон и выбирает снимок из галереи
прямо в админке. **Сжимает браузер заведения, а не воркер:** кадр с камеры
весит 3–5 МБ, и гонять такое по сети незачем. Перед отправкой он ужимается
до 900 точек по длинной стороне и в WebP — выходит около 100 КБ.

WebP умеют не все сборки браузеров. Если кодировщик отказал или **промолчал**,
снимок уходит в JPEG. Молчание тут не теория: в headless-сборке Edge
`canvas.toBlob` для webp не зовёт обратный вызов вообще, и без сторожа окно
заведения зависало бы на «Сжимаем…» навсегда.

Хранилище — **KV**, привязка `PHOTOS`. Не R2: бесплатного гигабайта хватает
с большим запасом, а R2 просит привязать карту даже на бесплатном тарифе.
Ограничения бесплатного KV: 1 ГБ, 1000 записей в сутки. Записи тратятся
только на загрузку снимков, чтения идут из кэша на краю сети.

Имя снимка содержит время загрузки, поэтому один адрес всегда означает одну
и ту же картинку — она кэшируется навсегда (`immutable`). Замена фотографии
у блюда даёт новый адрес; старый просто перестаёт запрашиваться и остаётся
в KV. Прибраться, если понадобится:

```bash
npx wrangler kv key list --binding PHOTOS --remote
npx wrangler kv key delete "photo:xamidoo:имя.webp" --binding PHOTOS --remote
```
